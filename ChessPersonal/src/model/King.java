...
public class King extends CheckPos{
	...
	public static boolean move(Character pieceColor, Character pieceName, int[] start, int[] destination)
	{
		...
		if(!Rook.move(pieceColor, pieceName, start, destination, 1, false) 
				&& !Bishop.move(pieceColor, pieceName, start, destination, 1, false))
		{
			return false;
		}
		board.updateSquares();
		...
		{
			Rook.move(pieceColor, pieceName, destination, start, 1, false);
			Bishop.move(pieceColor, pieceName, destination, start, 1, false);
			return false;
		}
